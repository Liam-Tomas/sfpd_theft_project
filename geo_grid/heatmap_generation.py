...
def analyze_incidents(joined, filter_condition, category_name, output_paths, grid):
    ...
    incident_counts = calculate_incident_probabilities(incident_data, total_incidents)
    average_monthly_incidents = calculate_average_incidents_per_month(incident_data)
    aggregated_info = aggregate_info(incident_data)
    
    # Save data and plot
    for path in output_paths[category_name]:
        merge_and_save_data(grid, incident_counts, average_monthly_incidents, aggregated_info, path)
...
def main():
    ...
    analyze_incidents(joined, (joined['Incident Category'] == 'Larceny Theft') & (joined['Incident Subcategory'] == 'Larceny - From Vehicle'), 'theft', output_paths, grid)
    analyze_incidents(joined, joined['Incident Description'] == 'Mental Health Detention', 'mental_health', output_paths, grid)
    analyze_incidents(joined, joined['Incident Category'] == 'Assault', 'assault', output_paths, grid)
    analyze_incidents(joined, joined['Incident Subcategory'] == 'Drug Violation', 'drugs', output_paths, grid)
    analyze_incidents(joined, joined['Incident Category'] == 'Burglary', 'burglary', output_paths, grid)
    analyze_incidents(joined, joined['Incident Category'] == 'Robbery', 'robbery', output_paths, grid)
